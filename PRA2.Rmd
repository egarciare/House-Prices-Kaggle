...
En el dataset Hi ha 1460 obs.amb 81 atributs, on la variable a predir és contínua (SalePrice). Addicionalment a la predicció dels preus, es fa un anàlisi per barris i una comparació dels preus de venda entre els anys 80 i els 90.
...
```{r vn_SaleCondition_, echo=FALSE}
...
priceYearBuilt <- summarize(group_by(train, YearBuilt), SalePriceMean = mean(SalePrice, na.rm=T), n=n())
...
priceYearRemodAdd <- summarize(group_by(train, YearRemodAdd), SalePriceMean = mean(SalePrice, na.rm=T), n=n())
...
priceGarageYrBlt <- summarize(group_by(train, GarageYrBlt), SalePriceMean = mean(SalePrice, na.rm=T), n=n())
...
priceYrSold <- summarize(group_by(train, YrSold), SalePriceMean = mean(SalePrice, na.rm=T), n=n())

...
En canvi passant el test de shapiro, la hipòtesi nul·la sí s'hauria de rebutjar:
```{r echo=TRUE, message=FALSE, warning=FALSE}
...
Segons el test de Wilcoxon s'observen diferències estadísticament significatives entre la venda de cases dels 80 i dels 90, ja que el p-value és inferior al valor de significació.



```{r echo=TRUE, message=FALSE, warning=FALSE}
# Plot
ggplot(train, aes(x=YearBuilt, y=SalePrice/1000)) +
  geom_line()
```
...
lmSalePrice4  = lm(SalePrice~FullBath+YearBuilt+Neighborhood+KitchenQual
                  +OverallQual+GarageFinish+GrLivArea+ExterQual+vnBsmtQual+TotalBsmtSF,data=train) 
...
El model amb un major coeficient de determinació és el model 4, capaç d'explicar la variabilitat de les dades en un 81,5% (Adjusted R-Squared).
...
newHouse <- data.frame( FullBath = 2, YearBuilt = 2003, Neighborhood = "CollgCr",
                       ...
                       GrLivArea=1710, ExterQual ="Gd",vnBsmtQual=5, TotalBsmtSF=856)
...
head(subset(train, select = c(FullBath,YearBuilt,Neighborhood,KitchenQual
                          ,OverallQual,GarageFinish,GrLivArea,ExterQual,vnBsmtQual,TotalBsmtSF,SalePrice) ),1)
```

Distància de cook del model, s'observa un punt molt allunyat del model.
```{r echo=TRUE, message=FALSE, warning=FALSE}

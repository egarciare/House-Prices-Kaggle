...
En els atributs categòrics sí s'aprecien valors NA, en el cas de Fence, els NA equivaldrien a no fence, i s'observa que en tots els casos formarien part del domini dels atributs. e.g. significant cap.
...
Grup de barri (segons la mitja de preus de venda. Grup 1 amb cases més barate, grup 3 amb les cases més cares) i el barri en questió.
...
s'analitza visualment per vnOverallQual (com a ratis en quant acabats de la casa) els barris relacionats.
...
Es comprova la normalitat, donat que serien subsets de la variable TotalPrice. S'usa el mètode de Kolmogorov-Smirnov 
...
Assumint com a hipòtesi nul·la que la població està distribuïda normalment, si el p-valor és més petit que el nivell de significació,(per defecte α=0,05) la hipòtesi nul·la es rebutjaria, però aquest no seria el cas ja que p-value és major. Per tant la distribució és normal.

Es comprova si en la decada dels 90 també els preus segueixen una distribució normal
...
Segons Kolmogorov-Smirnov, la distribució no seria normal. Es comprova com tampoc ho és seguint Shapiro-Wilk.
```{r echo=TRUE, message=FALSE, warning=FALSE}
...
#rlnorm(train.SalePrice90s)

train.SalePrice90s.norm <- BoxCox(train.SalePrice90s, lambda = BoxCoxLambda(train.SalePrice90s)) 
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
lmSalePrice  = lm(SalePrice~OverallQual+YearBuilt+vnNeighborhood+vnKitchenQual
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
lmSalePrice  = lm(SalePrice~OverallQual+YearBuilt+Neighborhood+KitchenQual
                  +FullBath+GarageCars+GarageArea+GrLivArea,data=train) 
summary(lmSalePrice)
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
lmSalePrice  = lm(SalePrice~FullBath+YearBuilt+vnNeighborhood+vnKitchenQual
                  +OverallQual+MasVnrArea,data=train) 
summary(lmSalePrice)
```
